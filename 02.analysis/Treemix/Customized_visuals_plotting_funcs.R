...
#2021/09/28: I changed settings of plot_tree and plot_tree_internal for more visible arrows
...
plot_tree_internal = function(d, e, o = NA, cex = 1, disp = 0.005, plus = 0.005, arrow = 0.05, ybar = 0.01, scale = T, mbar = T, mse = 0.01, plotmig = T, plotnames = T, xmin = 0, lwd = 1, font = 2){
	...
	text( -0.005, ybar + 0.04, lab = "10 s.e.", adj = 0, cex  = 1, font=1)
	lines( c(0, 0), c( ybar - 0.01, ybar+0.01))
	#lines( c(0.11, 0.11), c( ybar - 0.01, ybar+0.01))
	lines( c(mse*10, mse*10), c(ybar- 0.01, ybar+ 0.01))
	#lines( c(0.11+mse*10, 0.11+mse*10), c(ybar- 0.01, ybar+ 0.01))
	}
        if (mbar){
                mcols = rev( heat.colors(150) )
                ...
                w = l/100
                xma = max(d$x/20)
                rect( rep(-0.002, 100), ymi+(0:99)*w, rep(-0.001, 100), ymi+(1:100)*w, col = mcols, border = mcols)
                text(xma+disp-0.005, ymi, lab = "0", adj = 0, cex = 1, font=1)
		if ( mw >0.5){ text(xma+disp, yma, lab = "1", adj = 0, cex = 1, font=1)}
                else{
			text(xma+disp-0.005, yma, lab = "0.5", adj = 0, cex =1, font=1)
		}
		text(-0.003, yma+0.1, lab = "Migration", adj = 0 , cex = 1, font=2)
		text(-0.003, yma+0.05, lab = "weight", adj = 0 , cex = 1, font=2)
        }	
}
...
plot_tree = function(stem, o = NA, cex = 1.5, disp = 0.003, plus = 0.01, flip = vector(), arrow = 0.2, scale = T, ybar = 0.1, mbar = T, plotmig = T, plotnames = T, xmin = -0.006, lwd = 3, font = 2){
...
number_resid = function(stem, pop_order, min = -0.009, max = 0.009, cex = 1, usemax = T, wcols = "r"){
